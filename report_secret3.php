...
<body class="hold-transition  sidebar-mini layout-fixed layout-navbar-fixed layout-footer-fixed">
               ...
               with font-awesome or any other icon font library -->
                    ...
                    <li class="nav-item menu-open">
                        ...
                        <ul class="nav nav-treeview">
                            ...
                            <li class="nav-item">
                                <a href="./report_secret3.php" class="nav-link active">
                                    <i class="far fa-circle nav-icon" style="color: aqua;"></i>
                                    <p>เรื่องลับที่สุด</p>
                                </a>
                            </li>
    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            ...
            <table id="tbreport" class="display" style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th>วันที่ออกเลข</th>
                        <?php
                        if ($_SESSION['userType'] == 'Admin') echo "<th></th>";
                        ?>
                    </tr>
                </thead>
                <tbody>
                    <?php
                    $year = $_GET['year'];
                    $table = report("doc_secret3", $year);
                    foreach ($table as $tb) {
                        $nt = array();
                        $nt = explode(",", $tb['note']);
                        $ntsr = "";
                        if (str_contains($tb['note'], "อื่นๆ") > 0) {
                            $ntsr = "<ul><li>{$nt[count($nt) - 1]}</li></ul>";
                        }
                        $html = "<ul>";
                        for ($i = 0; $i < count($nt); $i++) {
                            if ($nt[$i] != "อื่นๆ") $html .= "<li>{$nt[$i]}</li>";
                            else {
                                $html .= "<li>{$nt[$i]} {$ntsr}</li>";
                            }
                        }
                        $html .= "</ul>";
                    ?>
                        <tr style="vertical-align: top;" class="t<?= $tb['id']; ?>">
                            ...
                            ?>
                                ...
                                ?>
                                    <td align="center">
                                        <button type="button" class="btn btn-info btn-sm btn-edit" id="btn-edit" data-id="<?= $tb['id']; ?>" data-num_doc="<?= $tb['num_doc']; ?>" onclick="edit('<?= $tb['id']; ?>')">
            ...
            <center>
                <button class="btn btn-secondary btn-sm" style="width: 250px; font-size:25px" onclick="window.open('print.php?type=secret3&year='+$('.years').val())" target="popup">Print</button>
            </center>
            <br><br>
        </section>
        <!-- /.content -->
...
<script>
    ...
    function edit(id) {
        window.location.href = "frm_book.php?type=secret3&id=" + id;
    }
    ...
    function del(id, num_doc) {
        Swal.fire({
            ...
            if (result.value) {
                $.get('report_del.php?type=secret3&id=' + id, function(res) {
                    if (res == '0') {
